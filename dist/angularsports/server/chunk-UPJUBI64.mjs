...
var T_VALUES = [-0.06405689286260563, 0.06405689286260563, -0.1911188674736163, 0.1911188674736163, -0.3150426796961634, 0.3150426796961634, -0.4337935076260451, 0.4337935076260451, -0.5454214713888396, 0.5454214713888396, -0.6480936519369755, 0.6480936519369755, -0.7401241915785544, 0.7401241915785544, -0.820001985973903, 0.820001985973903, -0.8864155270044011, 0.8864155270044011, -0.9382745520027328, 0.9382745520027328, -0.9747285559713095, 0.9747285559713095, -0.9951872199970213, 0.9951872199970213];
var C_VALUES = [0.12793819534675216, 0.12793819534675216, 0.1258374563468283, 0.1258374563468283, 0.12167047292780339, 0.12167047292780339, 0.1155056680537256, 0.1155056680537256, 0.10744427011596563, 0.10744427011596563, 0.09761865210411388, 0.09761865210411388, 0.08619016153195327, 0.08619016153195327, 0.0733464814110803, 0.0733464814110803, 0.05929858491543678, 0.05929858491543678, 0.04427743881741981, 0.04427743881741981, 0.028531388628933663, 0.028531388628933663, 0.0123412297999872, 0.0123412297999872];
...
var Post = class {
  ...
  addPass({
    vertex: vertex9 = defaultVertex,
    fragment: fragment8 = defaultFragment,
    uniforms = {},
    textureUniform = "tMap",
    enabled = true
  } = {}) {
...
var GPGPU = class {
  ...
  addPass({
    vertex: vertex9 = defaultVertex2,
    fragment: fragment8 = defaultFragment2,
    uniforms = {},
    textureUniform = "tMap",
    enabled = true
  } = {}) {
...
var GLTFLoader = class {
  ...
  static load(gl, src) {
    return __async(this, null, function* () {
      ...
      const desc = yield this.parseDesc(src);
      return this.parse(gl, desc, dir);
    });
  }
  static parse(gl, desc, dir) {
    return __async(this, null, function* () {
      ...
      const buffers = yield this.loadBuffers(desc, dir);
      gl.renderer.bindVertexArray(null);
      const bufferViews = this.parseBufferViews(gl, desc, buffers);
      const images = yield this.parseImages(gl, desc, dir, bufferViews);
      const textures = this.parseTextures(gl, desc, images);
      const materials = this.parseMaterials(gl, desc, textures);
      const skins = this.parseSkins(gl, desc, bufferViews);
      const meshes = yield this.parseMeshes(gl, desc, bufferViews, materials, skins);
  ...
  static parseImages(gl, desc, dir, bufferViews) {
    ...
    return Promise.all(desc.images.map((_0) => __async(this, [_0], function* ({
      uri,
      bufferView: bufferViewIndex,
      mimeType,
      name
      ...
      if (mimeType === "image/ktx2") {
        ...
        const image2 = yield this.basisManager.parseTexture(data);
        return image2;
      }
  ...
  static parseMeshes(gl, desc, bufferViews, materials, skins) {
    ...
    return Promise.all(desc.meshes.map((_0, _1) => __async(this, [_0, _1], function* ({
      primitives,
      // required
      weights,
      // optional
      ...
      if (isSkin) {
        primitives = yield Promise.all(skinIndices.map((skinIndex) => __async(this, null, function* () {
          return (yield this.parsePrimitives(gl, primitives, desc, bufferViews, materials, 1, isLightmap)).map(({
            geometry,
            program,
            mode
          }) => {
        ...
        primitives = (yield this.parsePrimitives(gl, primitives, desc, bufferViews, materials, numInstances, isLightmap)).map(({
          geometry,
          program,
          mode
        }) => {
  ...
  static parsePrimitives(gl, primitives, desc, bufferViews, materials, numInstances, isLightmap) {
    return Promise.all(primitives.map((_0) => __async(this, [_0], function* ({
      attributes,
      // required
      indices,
      // optional
      ...
      if (extensions && extensions.KHR_draco_mesh_compression) {
        ...
        const geometryData = yield this.dracoManager.decodeGeometry(data, {
...
var WireMesh = class extends Mesh {
  constructor(gl, _a = {}) {
    ...
    const wireGeometry = new Geometry(gl, {
      position: __spreadValues({}, geometry.attributes.position),
      index: {
        data: indicesTyped
      }
    });
    super(gl, __spreadProps(__spreadValues({}, meshProps), {
      mode: gl.LINES,
...
var Texture3D = class extends Texture {
  constructor(gl, args) {
    super(gl, __spreadProps(__spreadValues({}, args), {
...
//# sourceMappingURL=chunk-UPJUBI64.mjs.map
